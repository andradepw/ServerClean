...
RegisterCommand("livery", function(source, args, rawCommand)
    if IsPedInAnyVehicle(PlayerPedId()) then
        local selectedLivery = tonumber(args[1])
        if selectedLivery then
            SetVehicleLivery(GetVehiclePedIsIn(PlayerPedId()), selectedLivery)
        else
            TriggerEvent("chatMessage", "Sistema", {255, 0, 0}, "Use: /livery[número da plotagem]")
        end
    else
        TriggerEvent("chatMessage", "Sistema", {255, 0, 0}, "Você precisa estar em um veículo e usar: /livery[número da plotagem]")
    end
end, false) 
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADOPEN
